...
void signal_callback_handler(int signum) {
   cout << "grabette_slam.cc Caught signal " << signum << endl;
   exit(signum);
}
...
int main(int argc, char **argv) {
  ...
  // Grabette-specific outputs
  std::string output_gravity;
  app.add_option("--output_gravity", output_gravity, "Output gravity rotation matrix (3x3 CSV)");
  ...
  // Output gravity rotation matrix (3x3, row-major, comma-separated)
  if (!output_gravity.empty()) {
    if (SLAM.isImuInitialized()) {
      Eigen::Matrix3d Rwg = SLAM.GetGravityRotation();
      ofstream f(output_gravity);
      ...
      cerr << "Warning: IMU never initialized, skipping gravity output" << endl;
    }
  }

  // Output IMU biases (one line: bax,bay,baz,bwx,bwy,bwz)
  if (!output_biases.empty()) {
    if (SLAM.isImuInitialized()) {
      ORB_SLAM3::IMU::Bias b = SLAM.GetImuBiases();
      ofstream f(output_biases);
      f << fixed << setprecision(9);
      f << b.bax << "," << b.bay << "," << b.baz << ","
        << b.bwx << "," << b.bwy << "," << b.bwz << "\n";
      f.close();
